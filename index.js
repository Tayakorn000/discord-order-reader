...
if (typeof globalThis.fetch === 'undefined') {
  try {
    ...
    // Optional: Log internal polyfill failure
  }
}
...
process.on('unhandledRejection', err => {
  writeLog(`UNHANDLED PROMISE REJECTION: ${err?.stack || err}`);
});
...
async function ensureFont() {
  ...
  for (const { path: fp, url } of downloads) {
    if (!fs.existsSync(fp)) {
      try {
        console.log(`Fetching font: ${path.basename(fp)}...`);
...
function saveDb(db) {
  try {
    ...
    console.error('Failed to persist database changes.');
  }
}
...
function parseExcel(buffer) {
  ...
  if (!found) return { error: 'Required column ("พัสดุ" or "หมายเลขพัสดุ") not found.' };
...
async function sharpToImageData(input, options = {}) {
  try {
    ...
    // Convert 1-channel grayscale to 4-channel RGBA for decoders
...
async function scan(imageBuffer) {
  ...
  for (const id of stage1) {
    if (!id) continue;
    const r = await decodeAny(id);
    if (r) return { text: r, type: 'Code' };
  }

  // STAGE 2: Strategic Zone Cropping
  const zones = [
    [0, 0, BW * 0.6, BH * 0.6], [BW * 0.4, 0, BW * 0.6, BH * 0.6],
    [0, BH * 0.4, BW * 0.6, BH * 0.6], [BW * 0.4, BH * 0.4, BW * 0.6, BH * 0.6],
    [BW * 0.1, BH * 0.1, BW * 0.8, BH * 0.8], [0, BH * 0.2, BW, BH * 0.6],
    [BW * 0.15, BH * 0.25, BW * 0.7, BH * 0.5], [0, 0, BW * 0.55, BH * 0.55],
  ...
  const stage2 = await Promise.all(zones.map(async ([x, y, w, h]) => {
    ...
    if (!crop) return null;
    return decodeCrop(crop, Math.min(Math.round(w) * 3, 1400));
  }));
...
async function generatePDF(db) {
  ...
  const COLS = [
    { label: 'หมายเลขพัสดุ', w: 148 }, { label: 'ตรวจรับ', w: 148 },
    { label: 'จับคู่', w: 72 }, { label: 'ภาพถ่าย', w: 82 },
    { label: 'เวลา', w: 85 },
  ];
  ...
  y += 50;
  doc.font(fontBold).fontSize(40).fillColor('#000000').text('รบกวนพี่ๆขนส่งเซ็นต์รับด้วยนะคะ', ML, y, { width: TABLE_W, align: 'center' });
  y += 60;
  doc.font(font).fontSize(40).fillColor('#000000').text('(................................................)', ML, y, { width: TABLE_W, align: 'center' });
...
const SEEN_TTL = 300000;

function markSeen(id) {
  _seenMessages.set(id, Date.now());
  for (const [k, t] of _seenMessages) {
    if (Date.now() - t > SEEN_TTL) _seenMessages.delete(k);
  }
}

client.on(Events.MessageCreate, async (message) => {
  if (isShuttingDown || message.author.bot || message.channelId !== CHANNEL_ID) return;
  ...
  if (text === 'pdf') {
    try {
      ...
      if (!total) { await message.reply('❌ No data found.'); return; }
      ...
      await message.reply({
        content: `📄 **Report**\n✅ Received: **${received}** | ⏳ Pending: **${total - received}**`,
        files: [new AttachmentBuilder(pdfBuf, { name: `Report_${date}.pdf` })],
      });
    } catch (err) {
      await message.reply('❌ Failed to generate PDF.');
    }
    return;
  }
  ...
  if (text === 'สรุป' || text === 'summary') {
    try {
      ...
      const received = Object.values(db.parcels).filter(v => v.received).length;
      await message.reply({
        content: `📊 **Summary**\n📦 Total: **${total}** | ✅ Received: **${received}** | ⏳ Pending: **${total - received}**`,
        files: [new AttachmentBuilder(buffer, { name: `Summary_${date}.xlsx` })],
      });
    } catch (err) {
  ...
  if (text === 'clear') {
    try {
      ...
      if (Sheets.SHEET_ID()) {
        ...
        await message.reply(`🗑️ Data cleared. (Sheet: **${result.tab}**, Removed: **${result.cleared}**)`);
        return;
      }
      await message.reply(`🗑️ Cleared **${cleared}** local records.`);
    } catch (err) {
      await message.reply('❌ Failed to clear data.');
    }
    return;
  }
  ...
  if (text === '!status') {
    ...
    const total = Object.keys(db.parcels).length;
    const received = Object.values(db.parcels).filter(v => v.received).length;
    await message.reply(`📊 Total: **${total}** | ✅ Received: **${received}** | ⏳ Pending: **${total - received}**`);
    return;
  }
  ...
  if (xlsxFile) {
    try {
      ...
      if (!trackingNumbers.length) { await message.reply('❌ No data found in file.'); return; }
      ...
      if (Sheets.SHEET_ID()) {
        try {
          ...
          sheetsMsg = `\n☁️ Sheets Updated: **${tab}** (+${added})`;
        } catch (e) {
          sheetsMsg = `\n⚠️ Sheets Sync Error: ${e.message}`;
        }
      }

      await message.reply(`✅ Success: Ingested **${trackingNumbers.length}** parcels. (New: **${addedLocal}**)${sheetsMsg}`);
    } catch (err) {
      await message.reply('❌ Failed to process Excel file.');
    }
    return;
  }
  ...
  for (const { imageFile, buffer, found } of scanResults) {
    if (!found) { noRead.push(imageFile.name ?? 'Unknown'); continue; }
    const { text: code, type } = found;

    try {
      if (Sheets.SHEET_ID()) {
        const parcel = await Sheets.lookupParcel(code);
        if (!parcel) { results.push({ code, type, status: 'notfound' }); continue; }
        if (parcel.received) { results.push({ code, type, status: 'already', tab: parcel.tab }); continue; }
        ...
        if (!parcel) { results.push({ code, type, status: 'notfound' }); }
        else if (parcel.received) { results.push({ code, type, status: 'already' }); }
        else {
      ...
      noRead.push(imageFile.name ?? 'Unknown');
    }
  }

  const lines = results.map(r => {
    const icon = r.status === 'ok' ? '✅' : r.status === 'already' ? '🔁' : '❌';
    return `${icon} \`${r.code}\`${r.tab ? ` — ${r.tab}` : ''}${r.status === 'already' ? ' *(Already Received)*' : ''}`;
  });
  noRead.forEach(name => lines.push(`⚠️ Unreadable: ${name}`));

  const okCount = results.filter(r => r.status === 'ok').length;
  await message.reply(`📦 Scanned **${imageFiles.length}** | ✅ Success **${okCount}** | ❌ Fail **${noRead.length}**\n${lines.join('\n')}`);
});
